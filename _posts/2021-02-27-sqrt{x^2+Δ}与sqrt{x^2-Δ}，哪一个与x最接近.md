...
\( \sqrt{x^2+Δ} \)与\(\ sqrt{x^2-Δ} \)，哪一个与x最接近
...
当\\(\\Delta  = 0\\)时，\\(J = 0\\)，\\(H - x = x - L\\).  
当\\(\\Delta  > 0\\)时，  
\\(\\begin{array}{l}\\frac{J}{2} = \\sqrt \{\{x^4} - {\\Delta ^2}}  - {x^2}.\\\\{x^4} - {\\Delta ^2} < {x^4} \\Rightarrow \\sqrt \{\{x^4} - {\\Delta ^2}}  - {x^2} < 0\\\\ \\Rightarrow J = 2(\\sqrt \{\{x^4} - {\\Delta ^2}}  - {x^2}) < 0.\\\\ \\Rightarrow H - x < x - L.\\end{array}\\)
...
\\(\\therefore H - x = x - L\\).  
当\\(\\Delta  > 0\\)时，  
\\(\\begin{array}{l}H + x > L + x,\\\\\\therefore \\frac{\\Delta }\{\{H - x}} > \\frac{\\Delta }\{\{x - L}}.\\\\\\therefore H - x < x - L.\\end{array}\\)
...
\\[\\begin{array}{l}\\therefore {L^2} = {x^2} - \\Delta  = {x^2} - 2mx + {m^2}.\\\\\\therefore \\Delta  = 2mx - {m^2}.\\\\{H^2} = {x^2} + \\Delta  = {L^2} + 2\\Delta  = {x^2} + 2mx - {m^2} \\Rightarrow H = \\sqrt \{\{\{(x + m)}^2} - 2{m^2}}  < \\sqrt \{\{\{(x + m)}^2}}  = x + m = N\\\\ \\Rightarrow H < N \\Leftrightarrow H - x < N - x.\\\\N - x = m = x - L \\Rightarrow H - x < x - L.\\end{array}\\]
